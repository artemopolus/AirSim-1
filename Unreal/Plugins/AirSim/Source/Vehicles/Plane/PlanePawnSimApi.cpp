...
PlanePawnSimApi::PlanePawnSimApi(const Params& params)
    : PawnSimApi(params),
      pawn_events_(static_cast<PlanePawnEvents*>(params.pawn_events))
{
    //reset roll & pitch of vehicle as Planes required to be on plain surface at start
    Pose pose = getPose();
...
void PlanePawnSimApi::initialize()
{
...
void PlanePawnSimApi::pawnTick(float dt)
{
    unused(dt);
    //calls to update* are handled by physics engine and in SimModeWorldBase
}

void PlanePawnSimApi::updateRenderedState(float dt)
{
...
void PlanePawnSimApi::updateRendering(float dt)
{
...
void PlanePawnSimApi::setPose(const Pose& pose, bool ignore_collision)
{
...
void PlanePawnSimApi::resetImplementation()
{
...
void PlanePawnSimApi::reportState(StateReporter& reporter)
{
...
PlanePawnSimApi::UpdatableObject* PlanePawnSimApi::getPhysicsBody()
{
