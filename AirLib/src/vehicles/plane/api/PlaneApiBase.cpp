...
namespace msr {
	namespace airlib {
		...
		void PlaneApiBase::resetImplementation()
		{
			cancelLastTask();
			SingleTaskCall lock(this); //cancel previous tasks
		}

		bool PlaneApiBase::takeoff(float timeout_sec)
		{
		...
		bool PlaneApiBase::land(float timeout_sec)
		{
			SingleTaskCall lock(this);
		...
		bool PlaneApiBase::goHome(float timeout_sec)
		{
			SingleTaskCall lock(this);
		...
		bool PlaneApiBase::moveByAngleThrottle(float pitch, float roll, float throttle, float yaw_rate, float duration)
		{
			SingleTaskCall lock(this);
			...
			float lookahead, float adaptive_lookahead)
		{
			...
			if (velocity > getPlaneApiParams().breaking_vel) {
				breaking_dist = Utils::clip(velocity * getPlaneApiParams().vel_to_breaking_dist,
					getPlaneApiParams().min_breaking_dist, getPlaneApiParams().max_breaking_dist);
			}
			...
			while (!waiter.isTimeout() && (next_path_loc.seg_index < path_segs.size() - 1 || goal_dist > 0)
				...
				if (seg_velocity > getPlaneApiParams().min_vel_for_breaking && path_length_remaining <= breaking_dist) {
					seg_velocity = getPlaneApiParams().breaking_vel;
		...
		bool PlaneApiBase::moveToZ(float z, float velocity, float timeout_sec, const YawMode& yaw_mode,
			float lookahead, float adaptive_lookahead)
		{
			SingleTaskCall lock(this);
		...
		bool PlaneApiBase::moveByManual(float vx_max, float vy_max, float z_min, float duration, DrivetrainType drivetrain, const YawMode& yaw_mode)
		{
			...
			do {
				...
				if (rc_data.is_valid && (rc_data.timestamp == 0 || age <= kMaxMessageAge)) { //if rc message timestamp is not set OR is not too old 
					...
					catch (const PlaneApiBase::UnsafeMoveException& ex) {
		...
		bool PlaneApiBase::rotateToYaw(float yaw, float timeout_sec, float margin)
		{
			SingleTaskCall lock(this);
		...
		bool PlaneApiBase::rotateByYawRate(float yaw_rate, float duration)
		{
			SingleTaskCall lock(this);
		...
		void PlaneApiBase::moveByRC(const RCData& rc_data)
		{
			unused(rc_data);
			//by default we say that this command is not supported
			throw VehicleCommandNotImplementedException("moveByRC API is not implemented for this Plane");
		}

		void PlaneApiBase::moveByVelocityInternal(float vx, float vy, float vz, const YawMode& yaw_mode)
		{
			if (safetyCheckVelocity(Vector3r(vx, vy, vz)))
				commandVelocity(vx, vy, vz, yaw_mode);
		}

		void PlaneApiBase::moveByVelocityZInternal(float vx, float vy, float z, const YawMode& yaw_mode)
		{
			if (safetyCheckVelocityZ(vx, vy, z))
				commandVelocityZ(vx, vy, z, yaw_mode);
		}

		void PlaneApiBase::moveToPositionInternal(const Vector3r& dest, const YawMode& yaw_mode)
		{
			if (safetyCheckDestination(dest))
				commandPosition(dest.x(), dest.y(), dest.z(), yaw_mode);
		}

		void PlaneApiBase::moveByRollPitchThrottleInternal(float pitch, float roll, float throttle, float yaw_rate)
		{
			if (safetyCheckVelocity(getVelocity()))
				commandRollPitchThrottle(pitch, roll, throttle, yaw_rate);
		}

		void PlaneApiBase::moveByRollPitchZInternal(float pitch, float roll, float z, float yaw)
		{
			if (safetyCheckVelocity(getVelocity()))
		...
		Waiter PlaneApiBase::waitForFunction(WaitFunction function, float timeout_sec)
		{
			Waiter waiter(getCommandPeriod(), timeout_sec, getCancelToken());
			if (timeout_sec <= 0)
				return waiter;
		...
		bool PlaneApiBase::waitForZ(float timeout_sec, float z, float margin)
		{
			float cur_z = 100000;
			return waitForFunction([&]() {
				cur_z = getPosition().z();
		...
		void PlaneApiBase::setSafetyEval(const shared_ptr<SafetyEval> safety_eval_ptr)
		{
			SingleCall lock(this);
			safety_eval_ptr_ = safety_eval_ptr;
		}

		RCData PlaneApiBase::estimateRCTrims(float trimduration, float minCountForTrim, float maxTrim)
		{
		...
		bool PlaneApiBase::setSafety(SafetyEval::SafetyViolationType enable_reasons, float obs_clearance, SafetyEval::ObsAvoidanceStrategy obs_startegy,
		...
		bool PlaneApiBase::emergencyManeuverIfUnsafe(const SafetyEval::EvalResult& result)
		{
			if (!result.is_safe) {
		...
		bool PlaneApiBase::safetyCheckVelocity(const Vector3r& velocity)
		{
		...
		bool PlaneApiBase::safetyCheckVelocityZ(float vx, float vy, float z)
		{
			if (safety_eval_ptr_ == nullptr) //safety checks disabled
				return true;
		...
		bool PlaneApiBase::safetyCheckDestination(const Vector3r& dest_pos)
		{
			if (safety_eval_ptr_ == nullptr) //safety checks disabled
				return true;
		...
		float PlaneApiBase::setNextPathPosition(const vector<Vector3r>& path, const vector<PathSegment>& path_segs,
		...
		bool PlaneApiBase::isYawWithinMargin(float yaw_target, float margin) const
		{
			const float yaw_current = VectorMath::getYaw(getOrientation()) * 180 / M_PIf;
			return std::abs(yaw_current - yaw_target) <= margin;
		}

		float PlaneApiBase::getAutoLookahead(float velocity, float adaptive_lookahead,
		...
		float PlaneApiBase::getObsAvoidanceVelocity(float risk_dist, float max_obs_avoidance_vel) const
		{
			unused(risk_dist);
