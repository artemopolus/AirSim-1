...
PlaneRpcLibServer::PlaneRpcLibServer(ApiProvider* api_provider, string server_address, uint16_t port)
        : RpcLibServerBase(api_provider, server_address, port)
{
    ...
    (static_cast<rpc::server*>(getServer()))->
        bind("moveToZ", [&](float z, float velocity, float timeout_sec, const PlaneRpcLibAdapators::YawMode& yaw_mode, 
            float lookahead, float adaptive_lookahead, const std::string& vehicle_name) -> bool {
    ...
    (static_cast<rpc::server*>(getServer()))->
        bind("moveByRC", [&](const PlaneRpcLibAdapators::RCData& data, const std::string& vehicle_name) -> void {
    ...
    (static_cast<rpc::server*>(getServer()))->
        ...
        float obs_avoidance_vel, const PlaneRpcLibAdapators::Vector3r& origin, float xy_length, 
    ...
    //getters
    (static_cast<rpc::server*>(getServer()))->
        bind("getPlaneState", [&](const std::string& vehicle_name) -> PlaneRpcLibAdapators::PlaneState {
        return PlaneRpcLibAdapators::PlaneState(getVehicleApi(vehicle_name)->getPlaneState()); 
    });
}

//required for pimpl
PlaneRpcLibServer::~PlaneRpcLibServer()
{
}
