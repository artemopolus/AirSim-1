...
#ifndef air_neControlServer_hpp
#define air_PlaneControlServer_hpp

#include "common/Common.hpp"
#include "PlaneCommon.hpp"
...
namespace msr {
	namespace airlib {
		...
		class PlaneApiBase : public VehicleApiBase {
		...
		public: //these APIs uses above low level APIs
			...
			PlaneState getPlaneState() const
			{
				PlaneState state;
		...
		protected: //types
			...
			{
			public:
				SingleTaskCall(PlaneApiBase* api)
					: SingleCall(api)
				{
					if (isRootCall())
						api->beforeTask();
			...
			struct StatusLock {
				...
				StatusLock(const PlaneApiBase* api)
					: lock_(
