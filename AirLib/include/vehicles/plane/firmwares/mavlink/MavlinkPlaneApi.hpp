...
#ifndef msr_airlib_MavLinkPlaneController_hpp
#define msr_airlib_MavLinkPlaneController_hpp
...
namespace msr {
	namespace airlib {
		...
		class MavLinkPlaneApi : public PlaneApiBase
		{
		public: //methods
			virtual ~MavLinkPlaneApi()
			{
				closeAllConnection();
			...
			{
				PlaneApiBase::resetImplementation();
		...
		protected: //methods
			...
			//TODO: decouple PlaneApiBase, VehicalParams and SafetyEval
			virtual const PlaneApiParams& getPlaneApiParams() const override
			{
				//defaults are good for PX4 generic quadcopter.
				static const PlaneApiParams vehicle_params_;
		...
		protected: //methods
			virtual void connect()
			{
				if (!connecting_) {
					...
					this->connect_thread_ = std::thread(&MavLinkPlaneApi::connect_thread, this);
				}
			}
		...
		private: //methods
			...
			{
				if (connection_ == nullptr)
					throw std::domain_error("MavLinkPlaneApi requires connection object to be set before createProxy call");
		...
		private: //variables
			...
			std::shared_ptr<PlaneApiBase> mav_vehicle_control_;
			...
			//additional variables required for PlaneApiBase implementation
